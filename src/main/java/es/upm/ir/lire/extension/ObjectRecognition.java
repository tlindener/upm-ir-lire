...
public class ObjectRecognition {
	private static ObjectRecognition instance = null;

	protected ObjectRecognition() {
		// Exists only to defeat instantiation.
	}

	public static ObjectRecognition getInstance() {
		if (instance == null) {
			instance = new ObjectRecognition();
		}
		return instance;
	}

	public List<ObjectRecognitionResult> identifyObjects(String imagePath) {
		...
		List<ObjectRecognitionResult> results = null;
		try (Tensor image = constructAndExecuteGraphToNormalizeImage(imageBytes)) {
			results = new ArrayList<ObjectRecognitionResult>();
			float[] labelProbabilities = executeInceptionGraph(graphDef, image);
			for (int i = 0; i < 5; i++) {
				int bestLabelIdx = maxIndex(labelProbabilities);
				results.add(
						new ObjectRecognitionResult(labels.get(bestLabelIdx), labelProbabilities[bestLabelIdx] * 100f));
